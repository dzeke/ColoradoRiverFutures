---
title: "Lower Basin Water Uses from CRSS"
...
This is an R Markdown document. This document helps visualize Lower Basin consumptive uses from input data for Colorado River Simulation System model (CRSS)(version January 2021). Three plots show:
...
## Requested Citation:
Rosenberg (2021). "Lower Basin Water Uses from CRSS". https://github.com/dzeke/ColoradoRiverFutures/tree/master/LowerBasinAgUses.

```{r LBAgplots, echo=FALSE, warning=FALSE, message=FALSE}
...
\newpage
# Figure 1. Monthly consumptive water use requests
...
ggplot() +
  ...
  theme(text = element_text(size=nAxisFontSize),  legend.title = element_blank(), 
        legend.text=element_text(size=nLegendFontSize)) #,
        #legend.position= c(1.075,0.5))

```

\newpage
# Figure 2. Annual consumptive water use requests
...
ggplot() +
  ...
  scale_x_continuous(limits = c(2020,2030), breaks = seq(2020,2030, by=4)) +
  #scale_y_continuous(breaks=seq(0,3,by=1),labels=seq(0,3,by=1), sec.axis = sec_axis(~. +0, name = "", breaks = c(nMaxBalance$Total[2])/1e6, labels = c("Max Balance"))) +
...
\newpage
# Figure 3. Seasonal consumptive water use requests
